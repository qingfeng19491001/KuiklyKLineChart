...
class KLineRenderPlanner(registry: KLineExtensionRegistry) {
    ...
    fun create(
        ...
        val crosshair = snapshot.crosshair?.takeIf { mode.crosshair && it.index in range && paneById.containsKey(it.paneId) }?.let {
        ...
        return KLineRenderPlan(bounds, range, renderBars, panes, indicators, overlays, crosshair, tooltip, snapshot.theme, snapshot.formatters, viewport.barSpace, KLineRenderFeatures.from(mode))
    }

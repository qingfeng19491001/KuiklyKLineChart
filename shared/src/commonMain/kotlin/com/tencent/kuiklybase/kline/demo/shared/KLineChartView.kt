...
import com.tencent.kuiklybase.kline.demo.shared.canvas.KLineCanvasRenderer
...
class KLineChartDsl internal constructor() {
    ...
    internal var panes: MutableList<KLinePane> = mutableListOf(
        KLinePane(
            ...
            yAxes = listOf(KLineYAxis("price-y", 0.0, 100.0)),
        ),
        KLinePane(
            id = "volume", kind = KLinePaneKind.INDICATOR, order = 1, weight = 1.0, minHeight = 60.0,
            yAxes = listOf(KLineYAxis("volume-y", 0.0, 100.0)),
        ),
    )
    internal val indicators: MutableList<KLineIndicatorInstance> = mutableListOf()
